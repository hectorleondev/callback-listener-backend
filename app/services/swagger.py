...
def create_swagger_api(app):
    ...
    openapi_file_path = os.path.join(current_app.root_path, "..", "openapi.yaml")
    ...
    api = Api(
        ...
        description=openapi_spec.get("info", {}).get(
            "description", "A webhook capturing service API"
        ),
...
def setup_swagger_docs(api):
    ...
    webhooks_ns = Namespace(
        "webhooks", description="Webhook request capturing endpoints"
    )
    ...
    return {"paths": paths_ns, "webhooks": webhooks_ns, "health": health_ns}
